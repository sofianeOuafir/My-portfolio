  ...
  module('interestList').
    component('interestList', {
      ...
      controller: ['$http', '$translate', '$rootScope', function InterestListController($http, $translate, $rootScope){
        ...
        $http.get('http://localhost:8080/admin-cv/interest/list?lang=en').then(function(response){
        ...
        $rootScope.$on('$translateChangeSuccess', function () {
          ...
          {
            $http.get('http://localhost:8080/admin-cv/interest/list?lang=fr').then(function(response){
              self.interests = response.data;
            });
          }
